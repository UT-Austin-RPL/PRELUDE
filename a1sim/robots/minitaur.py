...
import re
import numpy as np
from a1sim.robots import minitaur_constants
from a1sim.robots import minitaur_motor
from a1sim.robots import robot_config
from a1sim.robots import action_filter
from a1sim.robots import kinematics
...
class Minitaur(object):
  ...
  def GetTrueLocalBaseVelocity(self):
    linear_velocity = self._pybullet_client.getBaseVelocity(self.quadruped)[0]
    _, orientation = self._pybullet_client.getBasePositionAndOrientation(self.quadruped)
    return self.TransformAngularVelocityToLocalFrame(linear_velocity,
                                                     orientation)
  ...
  def ApplyAction(self, motor_commands, motor_control_mode):
    ...
    # print('motor_model',self._motor_model)
